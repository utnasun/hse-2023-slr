"""
Source code link https://github.com/daniel-code/TubeViT/blob/main/tubevit/model.py
"""
...
class SparseTubesTokenizer(nn.Module):
    def __init__(self, hidden_dim, kernel_sizes, strides, offsets):
        ...
        self.conv_proj_bias = nn.Parameter(
            torch.zeros(len(self.kernel_sizes), self.hidden_dim), requires_grad=True
        )
        ...
        for i in range(len(self.kernel_sizes)):
            if i == 0:
                ...
                weight = F.interpolate(
                    self.conv_proj_weight, self.kernel_sizes[i], mode="trilinear"
                )
...
class SelfAttentionPooling(nn.Module):
    ...
    def forward(self, x):
        ...
        # (N, T, H) -> (N, T) -> (N, T, 1)
        att_w = nn.functional.softmax(self.W(x).squeeze(dim=-1), dim=-1).unsqueeze(
            dim=-1
        )
    ...
    def _generate_position_embedding(self) -> torch.nn.Parameter:
        ...
        for i in range(len(self.kernel_sizes)):
            tube_shape = self._calc_conv_shape(
                self.kernel_sizes[i], self.strides[i], self.offsets[i]
            )
