...
class BodyFeaturesExtractor:
    ...
    def _add_time_to_file_path(self, file_path: Path) -> Path:
    ...
    def extract_body_features(
        self, output_dir: Optional[Union[Path, str]] = None
    ) -> Tuple[Path, Path]:
        if not output_dir:
        ...
        json_file_name = self._add_time_to_file_path(
            Path("body_features_extractor_results.json")
        )
        ...
        with self.landmarker.create_from_options(self.landmarker_options) as landmarker:
            while True:
                ...
                landmarker_result = dataclasses.asdict(
                    landmarker.detect_for_video(mp_image, int(time.time() * 1000))
                )

                annotated_image = draw_landmarks_on_image(
                    mp_image.numpy_view(), landmarker_result, self.landmark_type
                )
