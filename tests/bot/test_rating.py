...
async def test_call_menu_handler():
    requester = MockedBot(MessageHandler(call_rating_menu))
...
@pytest.mark.parametrize(
    "call_back_num", ["num_one", "num_two", "num_three", "num_four", "num_five"]
)
@pytest.mark.asyncio
async def test_callbacks_num(call_back_num, monkeypatch):
    ...
    requester = MockedBot(CallbackQueryHandler(callbacks_num))
    ...
    callback_query = CALLBACK_QUERY.as_object(
        data=call_back_num,
        message=MESSAGE.as_object(text="Выберите оценку бота от 1 до 5"),
    )
    ...
    callback_query = CALLBACK_QUERY.as_object(
        ...
        message=MESSAGE.as_object(
            text=f'Ваша оценка: {dict_nums[call_back_num]}.\
...
async def test_show_rating():
    requester = MockedBot(MessageHandler(show_rating))

    calls = await requester.query(
        message=MESSAGE.as_object(text="Посмотреть рейтинг бота")
    )
    answer_message = calls.send_message.fetchone()
