...
from slr_bot.handlers.activity import (
    call_rating_menu,
    get_new_users_by_dow_count,
    get_num_unique_users,
)
...
async def test_call_rating_menu():
    ...
    calls = await request.query(
        message=MESSAGE.as_object(text="Получить аналитику об активности")
    )
...
async def test_get_num_unique_users():
    ...
    calls = await request.query(
        message=MESSAGE.as_object(text="Количество уникальных пользователей")
    )
    answer_message = calls.send_message.fetchone()
...
async def test_get_new_users_by_dow_count():
    ...
    calls = await request.query(
        message=MESSAGE.as_object(
            text="Суммарное количество новых пользователей по дням недели"
        )
    )
    answer_message = calls.send_message.fetchone()
