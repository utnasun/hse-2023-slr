...
async def call_label_menu(message: Message):
    await message.answer("Что вы хотите разметить?", reply_markup=get_label_menu())
...
async def suggest_video_upload(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Пожалуйста, загрузите видео файлом.", reply_markup=ReplyKeyboardRemove()
    )
...
async def call_body_labeler(message: Message, state: FSMContext, bot: Bot):
    ...
    landmarkers = {
        "Разметить руки": {
            ...
            "landmarker_options": HandLandmarkerOptions(
                base_options=BaseOptions(
                    model_asset_path=str(DATA_PATH / "hand_landmarker.task")
                ),
        ...
        "Разметить позу": {
            ...
            "landmarker_options": PoseLandmarkerOptions(
                base_options=BaseOptions(
                    model_asset_path=str(DATA_PATH / "pose_landmarker_heavy.task")
                ),
                running_mode=RunningMode.VIDEO,
                num_poses=1,
    ...
    body_features_extractor = BodyFeaturesExtractor(
        input_video_path=file_path, **landmarkers[landmark_type]
    )

    video_file_name, json_file_name = body_features_extractor.extract_body_features()

    await message.answer("Ожидайте завершения процесса разметки рук на видео.")
